...
import { createBolum, getSorular } from "@/lib/firestore";
import type { Soru } from "@/types";

export default function YeniBolumPage() {
  ...
  useEffect(() => { getSorular().then(setSorular); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    await createBolum({ ad: ad.trim(), aciklama: aciklama.trim(), soruIdleri: seciliIds });
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-sm font-semibold text-slate-800">Yeni Bölüm</span>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-white rounded-2xl border border-slate-100 p-6 space-y-5">
          <h1 className="text-lg font-bold text-slate-900">Yeni Bölüm</h1>
